...
/// </summary>
public class DefensiveAttackState : AttackState
{
    Vector3 pivot;
    ...
    public DefensiveAttackState(EntityAttack entityAttack, Vector3 pivot) : base(entityAttack)
    {
        this.pivot = pivot;
    }
    ...
    public override void UpdateState()
    {
        ...
        float distanceToPivot = Vector3.Distance(entityAttack.transform.position, pivot);
        if (distanceToPivot > entityAttack.Stats.HoldingRange)
        {
            ReturnToPivot();
            return;
        }

        //if our target is too far away, return to pivot
        Vector3 targetPosition = entityAttack.Target.UnitColider.ClosestPointOnBounds(entityAttack.transform.position);

        float distancePivotToTarget = Vector3.Distance(targetPosition, pivot);
        if(distancePivotToTarget > entityAttack.Stats.SearchRange)
        {
            ReturnToPivot();
            return;
        }
        //var targetDir = targetPosition - pivot;
        /*Vector3 predictedMovement = Vector3.Lerp(pivot, targetPosition - targetDir.normalized * entityAttack.Stats.Range, distancePivotToTarget / entityAttack.Stats.HoldingRange);
        float preditedDistance = Vector3.Distance(predictedMovement, targetPosition);

        if (preditedDistance > entityAttack.Stats.Range)
        {
            ReturnToPivot();
            return;
        }*/
